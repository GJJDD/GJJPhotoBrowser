...
@interface GJJPhotoBrowser () <UICollectionViewDataSource, UICollectionViewDelegate,UICollectionViewDelegateFlowLayout> {
...
@implementation GJJPhotoBrowser
...
+ (instancetype)showFromImageView:(UIImageView *)imageView withURLStrings:(NSArray *)URLStrings placeholderImage:(UIImage *)image atIndex:(NSInteger)index dismiss:(DismissBlock)block {
    GJJPhotoBrowser *browser = [[GJJPhotoBrowser alloc] initWithFrame:kScreenRect];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    GJJPhotoBrowserCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:kPhotoBrowserCellID forIndexPath:indexPath];
    ...
    if (self.URLStrings) {
        ...
        if (indexPath.row != _index) {
            [cell.imageView gjj_setImageWithURL:url placeholderImage:_placeholderImage];
        }
        else {
            UIImage *placeHolder = _tmpImageView.image;
            [cell.imageView gjj_setImageWithURL:url placeholderImage:placeHolder completed:^(UIImage *image, NSError *error, NSURL *imageUrl) {
                if (!_imageDidLoaded) {
                     _imageDidLoaded = YES;
...
- (void)configureBrowser {
    ...
    [self.collectionView registerClass:[GJJPhotoBrowserCell class] forCellWithReuseIdentifier:kPhotoBrowserCellID];
    
    [[UIApplication sharedApplication].keyWindow addSubview:self];
}

- (void)setupToolBar {
    _toolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-45, self.frame.size.width, 32)];
...
//    countLab.backgroundColor = [[UIColor blackColor]colorWithAlphaComponent:0.4];
    ...
    saveBtn.frame = CGRectMake(_toolBar.frame.size.width-58, 0, 32, 32);
    saveBtn.layer.cornerRadius = 2;
//    [saveBtn setBackgroundColor:[[UIColor blackColor]colorWithAlphaComponent:0.4]];
//    [saveBtn setTitle:@"保存" forState:UIControlStateNormal];
    [saveBtn setImage:[UIImage imageNamed:@"INBrowseDownload"] forState:(UIControlStateNormal)];
...
- (void)animateImageViewAtIndex:(NSInteger)index {
    ...
    if (self.URLStrings && !self.images) {
        NSString *key = [GJJWebImageDownloader cacheKeyForURL:[NSURL URLWithString:self.URLStrings[_index]]];
        UIImage *image = [GJJWebImageDownloader imageFromDiskCacheForKey:key];
...
//    CGFloat width = [text sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]}].width+8;
    _countLab.frame = CGRectMake(0, 30, kScreenWidth, 28);
